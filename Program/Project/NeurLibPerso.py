...
class NeuralNetwork:
    # stuff of dreams
    def __init__(self, config, nb_layers=None, activation_function=None):
        ...
        if nb_layers is not None:
            self.layers = nb_layers
        else:
            list_of_strings = self.config['structure']
            self.layers = list()
            for value in list_of_strings.split(", "):
                self.layers.append(int(value))

        if self.config['verbose'] and int(self.config['log_level']) > 0:
                print("\tNetwork Structure: " + str(self.layers))
    ...
    def generate_weights(self, nb_layers):
        ...
        for current_layer in nb_layers:
            if i < len(nb_layers) - 1:
                ...
                if self.config['verbose'] and int(self.config['log_level']) > 1:
                    print(DataFrame(weights[len(weights) - 1]))
            i += 1
        return weights
    ...
    def activate(self, value):
        if int(self.activation_function) is 0:
